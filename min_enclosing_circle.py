...
class MinEnclosingCircle:
    ...
    def fit(self):
        ...
        self.radius = np.linalg.norm(self.pts[0] - self.pts[1]) / 2
    ...
    def find_second_point(self, i):
        ...
        radius = np.linalg.norm(self.pts[0] - self.pts[i]) / 2
        for j in range(1, i):
            distance = np.linalg.norm(self.pts[j] - center)
            if distance >= radius + EPS:
                center, radius = self.find_third_point(i, j)
        return center, radius

    def find_third_point(self, i, j):
        center = (self.pts[j] + self.pts[i]) / 2
        radius = np.linalg.norm(self.pts[j] - self.pts[i]) / 2
        for k in range(j):
            distance = np.linalg.norm(self.pts[k] - center)
            if distance >= radius + EPS:
                center, radius = self.find_circle_3pts(i, j, k)
        return center, radius
    ...
    def find_circle_3pts(self, i, j, k):
        ...
        if abs(det) <= EPS:
            ...
            center = center / det
            radius = np.linalg.norm(center - self.pts[i])
        return center, radius
